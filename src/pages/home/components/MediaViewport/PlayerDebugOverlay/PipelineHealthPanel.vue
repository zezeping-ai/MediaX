...
const lanes = computed<HealthLane[]>(() => {
  ...
  return [
    buildNetworkLane(networkRatio, props.history),
    buildBudgetLane("decode", "解码", decodeAvg, frameBudgetMs, props.history),
    buildAudioLane(audioQueue, props.history),
    buildRenderLane(telemetry.gpu_queue_utilization ?? null, renderCost, frameBudgetMs, props.history),
  ];
});

function buildNetworkLane(ratio: number | null, history: Array<{ at_ms: number; telemetry: MediaTelemetryPayload }>): HealthLane {
...
function buildAudioLane(queueDepth: number | null, history: Array<{ at_ms: number; telemetry: MediaTelemetryPayload }>): HealthLane {
