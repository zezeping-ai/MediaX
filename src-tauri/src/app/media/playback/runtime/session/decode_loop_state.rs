...
use crate::app::media::playback::runtime::video_pipeline::{
    percentile_from_sorted, ProcessMetricsSampler, VideoFramePipeline,
};
use crate::app::media::playback::runtime::{
    METRICS_EMIT_INTERVAL_MS, RATE_SWITCH_SETTLE_WINDOW_MS,
};
    ...
    pub fn record_video_packet(
        ...
        emit_debug(
            ...
            format!(
                ...
                key_ratio,
                keyint_est,
                self.video_demux_packets,
                self.video_demux_bytes
            ),
        );
        emit_debug(
            app,
            "video_gop",
            self.build_gop_debug_message(),
        );
