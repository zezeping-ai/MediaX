...
use super::meter::{create_shared_audio_meter, spawn_audio_meter_emitter, MeteredSource};
...
impl AudioOutput {
    pub fn new(
        ...
        let meter_thread = spawn_audio_meter_emitter(
            app.clone(),
            meter_shared.clone(),
            meter_stop_flag.clone(),
        );
        let output = Self {
            _stream: stream,
            player,
    ...
    pub fn append_pcm_f32(&self, sample_rate: u32, channels: u16, pcm: &[f32]) {
        ...
        self.player
            .append(MeteredSource::new(source, self.meter_shared.clone(), self.controls.clone()));
    }
}
