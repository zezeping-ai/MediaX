...
use crate::app::media::snapshot::{emit_snapshot, snapshot_from_state};
...
#[tauri::command]
pub fn media_pause(app: AppHandle, state: State<'_, MediaState>) -> Result<MediaSnapshot, String> {
    ...
    if status == PlaybackStatus::Paused {
        ...
        if let Some(source) = path {
            ...
            if should_force_render {
                let app_handle = app.clone();
                if let Err(err) =
                    render_paused_seek_frame(&app_handle, &renderer, &source, target, epoch)
                {
