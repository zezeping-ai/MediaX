...
fn build(sysroot: Option<&str>) -> io::Result<()> {
    ...
    if target != host {
        ...
        // Redistributable static builds must not tune for the builder CPU: e.g. M4 CI
        // enables ARM I8MM via -march=native, which SIGILLs on M1/M2/M3 at runtime.
        let arch = env::var("CARGO_CFG_TARGET_ARCH").unwrap();
        if arch == "aarch64" {
            configure.arg("--extra-cflags=-march=armv8-a");
        } else {
    ...
    // Platform HW decode: explicit crate features, or bundled static builds on Apple (Release).
    // Dev without `bundled-ffmpeg` links system FFmpeg (already has VT/VAAPI when installed).
    let target_os = env::var("CARGO_CFG_TARGET_OS").unwrap();
    let bundled_build = env::var("CARGO_FEATURE_BUILD").is_ok();

    // Apple VideoToolbox (iOS and macOS)
    let enable_videotoolbox = env::var("CARGO_FEATURE_BUILD_VIDEOTOOLBOX").is_ok()
        || (bundled_build && matches!(target_os.as_str(), "ios" | "macos"));
    ...
    // Linux VAAPI: only when explicitly requested (needs libva dev headers in the build env).
    if env::var("CARGO_FEATURE_BUILD_VAAPI").is_ok() && target_os == "linux" {
